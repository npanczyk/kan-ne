...
def run_model(device, dataset, params, run_name, lib):
...
def test_libs(device, dataset, params, run_name):
        ...
        for i, lib in enumerate(libs):
            run_model(device, dataset, params, f'{run_name}_lib{i}', lib)
        return

if __name__=="__main__":
    os.environ["CUDA_VISIBLE_DEVICES"]="2"
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    test_libs(device, get_fp(cuda=True), fp_best, 'FP_250225')
    test_libs(device, get_bwr(cuda=True), bwr_best, 'BWR_250225')
    test_libs(device, get_heat(cuda=True), heat_best, 'HEAT_250225')
    test_libs(device, get_htgr(cuda=True), htgr_best, 'HTGR_250225')
    test_libs(device, get_chf(cuda=True), chf_best, 'CHF_250225')
    test_libs(device, get_rea(cuda=True), rea_best, 'REA_250225')
    test_libs(device, get_xs(cuda=True), xs_best, 'XS_250225')
